...
interface SnipeTimerProps {
  endTime: Date;
  snipeTime: number;
}

const SnipeTimer: React.FC<SnipeTimerProps> = ({ endTime, snipeTime }) => {
  ...
  useEffect(() => {
    const calculateTimeLeft = () => {
      const now = new Date();
      const snipeTimeMs = snipeTime * 1000;
      const diff = endTime.getTime() - now.getTime();
      const diffToSnipe = diff - snipeTimeMs;
      ...
      // Determine if we're close to snipe time
      const isSoon = diffToSnipe <= 60 * 1000; // Within 1 minute of snipe time
  ...
  const getTimerColor = () => {
    ...
    if (timeRemaining.isSoon) return 'text-red-500'; // Very close to snipe time
    ...
    <div className="space-y-1">
      ...
      <div className="text-xs text-right">
        {timeRemaining.total <= 0 ? (
          ...
          <span className="text-red-500 font-medium">Snipe imminent!</span>
        ) : (
          <span className="text-muted-foreground">Snipe will execute {snipeTime} seconds before end</span>
        )}
      </div>
    </div>
  );
